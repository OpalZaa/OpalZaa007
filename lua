...
local window = library:AddWindow("โอปอซ่าครับพี่ | หวัดดีครับพี่ " .. displayName, {
    main_color = Color3.fromRGB(255, 128, 0), -- เปลี่ยนเป็นโทนสีส้ม (Orange)
    min_size = Vector2.new(650, 870),
    can_resize = false,
})

local AutoFarm = window:AddTab("ฟาร์ม")
...
AutoFarm:AddSwitch("ฟามกล้ามแบบแพ็คกดตรงนี้ถ้าปิงเกิน250)", function(state)
...
-- Switch en la librería
AutoFarm:AddSwitch("ฟามกล้ามแบบแพ็คกดตรงนี้ถ้าปิงต่ำกว่า150)", function(state)
    getgenv()._AutoRepFarmEnabled = state
    warn("[Auto Rep Farm] Estado cambiado a:", state and "ON" or "OFF")
end)
...
AutoFarm:AddSwitch("กินไข่ทุกๆ 30 นาที", function(state)
    autoEatEnabled = state
end)


    AutoFarm:AddSwitch("หมุนวงล้อ", function(state)
...
-- ✅ Optimización visual
AutoFarm:AddSwitch("เปิดไว้ด้วยนะครับก่อนจะฟามกล้าม", function(bool)
...
AutoFarm:AddButton("กันแล็คธรรมดา", function()
...
AutoFarm:AddButton("กันแล็คแบบจอดำ แนะนำขึ้นทีฟามก่อนกดใช้นะครับ", function()
    local player = game.Players.LocalPlayer
...
AutoFarm:AddButton("กดใส่ซามูไร", function()
...
AutoFarm:AddButton("ที่ฟามกล้าม + ทนทาน", function()
    local player = game.Players.LocalPlayer
...
AutoFarm:AddButton("ที่ฟามกล้ามล้วน", function()
    local player = game.Players.LocalPlayer
    local char = player.Character or player.CharacterAdded:Wait()
    local hrp = char:WaitForChild("HumanoidRootPart")
...
AutoFarm:AddLabel("ช่องรีเบิร์ด").TextSize = 23


local fastStrengthFolder = AutoFarm:AddFolder("รีแบบแพ็ค")
...
local footerRebirthsGainedLabel = fastStrengthFolder:AddLabel("Rebirths ที่ฟามได้: 0")
...
local rebirthFolder = AutoFarm:AddFolder("Rebiths แบบไม่แพ็ค")
...
rebirthFolder:AddTextBox("เลือกเลขที่จะให้หยุดรี", function(value)
...
local targetRebirthToggle = rebirthFolder:AddSwitch("เริ่มรี", function(enabled)
...
rebirthFolder:AddSwitch("ออโต้วาปไปที่คิง", function(enabled)
    if enabled then
...
rebirthFolder:AddSwitch("ออโต้ขนาด 2", function(enabled)
    if enabled then
        _G.AutoSize2 = true
...
local autoEquipToolsFolder = AutoFarm:AddFolder("ออโต้เครื่องยก")

-- Botón para desbloquear el Gamepass AutoLift
autoEquipToolsFolder:AddButton("เกมพาส ยกออโต้", function()
...
autoEquipToolsFolder:AddSwitch("ยกเร็ว + ต่อยเร็ว", function(Value)
    _G.FastTools = Value
...
local estadisticas = window:AddTab("สตัด")

local SelectPlayerName = ""

local PlayerDrop = estadisticas:AddDropdown("เลือกชื่อคน", function(Value)
...
estadisticas:AddLabel("สตัด advance:").TextSize = 24

local enemyHealthLabel = estadisticas:AddLabel("เลือดศัตรู: ผมยังไม่ได้ทำ")
enemyHealthLabel.TextSize = 20
enemyHealthLabel.TextColor3 = Color3.fromRGB(255, 128, 0)

local playerDamageLabel = estadisticas:AddLabel("ดาเมจเรา: ผมยังไม่ได้ทำ")
playerDamageLabel.TextSize = 20
playerDamageLabel.TextColor3 = Color3.fromRGB(255, 128, 0)

local hitsToKillLabel = estadisticas:AddLabel("จำนวนหมัดที่ต้องต่อยถึงจะตาย: ผมยังไม่ได้ทำ")
hitsToKillLabel.TextSize = 20
hitsToKillLabel.TextColor3 = Color3.fromRGB(255, 128, 0)
...
local Killer = window:AddTab("ออโต้คิว")
...
local titleLabel = Killer:AddLabel("เลือกสัตว์ดาเมจหรือทนทาน (ตัวแพ็ค)")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local dropdown = Killer:AddDropdown("เลือกสัตว์", function(text)
...
Killer:AddSwitch("ออโต้ไวริสเพื่อนในเกม", function(state)
...
Killer:AddSwitch("ออโต้คิว", function(bool)
    autoKill = bool
...
local targetDropdown = Killer:AddDropdown("เลือกเป้าหมาย", function(displayName)
    for _, player in ipairs(Players:GetPlayers()) do
...
Killer:AddButton("ลบชื่อเป้าหมายที่เลือกอยู่", function()
...
Killer:AddSwitch("ฆ่าคนที่เลือก", function(state)
    killTarget = state
...
local spyTargetDropdown = Killer:AddDropdown("เลือกชื่อคนที่จะส่อง", function(displayName)
    for _, player in ipairs(Players:GetPlayers()) do
        if player.DisplayName == displayName then
...
Killer:AddSwitch("ส่องผู้เล่น", function(bool)
    spying = bool
    if not spying then
...
local button = Killer:AddButton("ลบอนิเมชั่นต่อย", function()
...
Killer:AddButton("คืนอนิเมชั่นต่อย", function()
    RecoveryPunch()
end)

Killer:AddSwitch("ออโต้ถือหมัด", function(state)
...
Killer:AddSwitch("ออโต้ต่อย แบบไม่มีอนิเมชั่น", function(state)
...
Killer:AddSwitch("ออโต้ต่อย", function(state)
...
Killer:AddSwitch("ต่อยเร็ว", function(state)
...
Killer:AddSwitch("อมตะ", function(State)
    godModeToggle = State
    if State then
        task.spawn(function()
...
local followDropdown = Killer:AddDropdown("วาปตามหลัง", function(selectedDisplayName)
...
Killer:AddButton("เลิกตาม", function()
    following = false
    followTarget = nil
...
Killer:AddSwitch("ออโต้ทุบ", function(state)
...
local button = Killer:AddButton("เดินบนน้ำ", function()
...
Killer:AddButton("ไซต์จิ๋ว", function()
...
Killer:AddButton("ต่อยตอนตาย", function()
...
-- Dropdown
local timeDropdown = Killer:AddDropdown("เลือกเวลา", function(selection)
...
local teleport = window:AddTab("วาปไปเกาะต่างๆ")
...
local Calculadora = window:AddTab("คำนวณเลือดดาเมจ", Color3.fromRGB(200, 100, 100))

local baseStrength = 0
local resultadoLabelsDamage = {}

local FolderDamage = Calculadora:AddFolder("แพ็คดาเมจคำนวณ")

FolderDamage:AddTextBox("ใส่เลข (เช่น: 1.55Qa, T, B)", function(text)
...
FolderDamage:AddButton("กดคำนวณ", function()
...
local FolderDurabilidad = Calculadora:AddFolder("แพ็คทนทานคำนวณ")

FolderDurabilidad:AddTextBox("ใส่เลข (เช่: 1.55Qa, T, B)", function(text)
    local unidades = { ["T"] = 1e12, ["Q"] = 1e15, ["B"] = 1e9 }
    text = text:upper()
    for u, m in pairs(unidades) do
        if text:find(u) then
...
FolderDurabilidad:AddButton("กดคำนวณทนทาน", function()
...
local features = window:AddTab("คำนวณ Stats หลัก")
features:Show()
...
local stopwatchLabel = features:AddLabel("เวลายกเร็ว: 0d 0h 0m 0s")
stopwatchLabel.TextSize = 20

local projectedStrengthLabel = features:AddLabel("กล้าม : 0 /ชั่วโมง | 0 /วัน | 0 /อาทิตย์ | 0 /เดือน")
projectedStrengthLabel.TextSize = 20

local projectedDurabilityLabel = features:AddLabel("ทนทาน : 0 /ชั่วโมง | 0 /วัน | 0 /อาทิตย์ | 0 /เดือน")
projectedDurabilityLabel.TextSize = 20

features:AddLabel("").TextSize = 10

local statsLabel = features:AddLabel("สตัด:")
statsLabel.TextSize = 24

local strengthLabel = features:AddLabel("กล้าม: 0 | ฟามได้: 0")
strengthLabel.TextSize = 20

local durabilityLabel = features:AddLabel("ทนทาน: 0 | ฟามได้: 0")
...
task.spawn(function()
    ...
    while true do
        ...
        if trackingStarted then
            ...
            stopwatchLabel.Text = string.format("เวลายกเร็ว: %dd %dh %dm %ds", days, hours, minutes, seconds)
            ...
            strengthLabel.Text = "กล้าม: " .. formatNumber(currentStrength) .. " | ฟามได้: " .. formatNumber(sessionStrengthDelta)
            durabilityLabel.Text = "ทนทาน: " .. formatNumber(currentDurability) .. " | ฟามได้: " .. formatNumber(sessionDurabilityDelta)
            ...
            if currentTime - lastCalcTime >= calculationInterval then
                ...
                if #strengthHistory >= 2 then
                    ...
                    projectedStrengthLabel.Text = "กล้าม : " .. formatNumber(strengthPerHour) .. "/ชั่วโมง | " .. formatNumber(strengthPerDay) .. "/วัน | " .. formatNumber(strengthPerWeek) .. "/อาทิตย์ | " .. formatNumber(strengthPerMonth) .. "/เดือน"
                end
                ...
                if #durabilityHistory >= 2 then
                    ...
                    projectedDurabilityLabel.Text = "ทนทาน : " .. formatNumber(durabilityPerHour) .. "/ชั่วโมง | " .. formatNumber(durabilityPerDay) .. "/วัน | " .. formatNumber(durabilityPerWeek) .. "/อาทิตย์ | " .. formatNumber(durabilityPerMonth) .. "/เดือน"
                end
            end
        end
...
local pets = window:AddTab(       "ซื้อสัตว์เลี้ยง")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
pets:AddLabel("ซื้อสัตว์เลี้ยงและออร่า")

-- Pet dropdown
local petDropdown = pets:AddDropdown("เลือกสัตว์เลี้ยง", function(text)
    selectedPet = text
...
-- Aura dropdown
local auraDropdown = pets:AddDropdown("เลือกออร่า", function(text)
    selectedAura = text
    local crystal = findCrystalForItem(text)
...
pets:AddLabel("กดซื้อ")

-- Auto buy pet toggle
pets:AddSwitch("กดซื้อสัตว์", function(bool)
    _G.AutoBuyPet = bool
...
pets:AddSwitch("กดซื้อออร่า", function(bool)
    _G.AutoBuyAura = bool
...
-- Ultimate dropdown
local ultimateDropdown = pets:AddDropdown("เลือก ultimates", function(text)
...
pets:AddSwitch("ออโต้แลก Ultimates", function(bool)
...
local farmTab = window:AddTab("หิน")
farmTab:AddLabel("ฟามหินทั้งหมดในเกม")
...
Gift:AddLabel("กิ๊ฟ Protein egg:").TextSize = 22

local proteinEggLabel = Gift:AddLabel("จำนวนที่มี Protein Eggs: 0")
proteinEggLabel.TextSize = 20

local selectedEggPlayer = nil
local eggCount = 0

local eggDropdown = Gift:AddDropdown("เลือกชื่อคนที่จะกิ๊ฟ", function(selectedDisplayName)
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr.DisplayName == selectedDisplayName then
...
Gift:AddTextBox("จำนวน", function(text)
    eggCount = tonumber(text) or 0
end)

Gift:AddButton("กิ๊ฟ Eggs", function()
...
Gift:AddLabel("กิ๊ฟน้ำสัปรด:").TextSize = 22

local tropicalShakeLabel = Gift:AddLabel("จำนวนที่มี สัปรด: 0")
tropicalShakeLabel.TextSize = 18

local selectedShakePlayer = nil
local shakeCount = 0

local shakeDropdown = Gift:AddDropdown("เลือกชื่อคนที่จะกิ๊ฟ", function(selectedDisplayName)
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr.DisplayName == selectedDisplayName then
...
Gift:AddTextBox("ที่กิ๊ฟน้ำสัปรด", function(text)
    shakeCount = tonumber(text) or 0
end)

Gift:AddButton("กิ๊ฟน้ำสัปกด", function()
...
local Credits = window:AddTab("เครดิต")

Credits:AddLabel("สคริปต์สร้าง By โอปอซ่า")
Credits:AddLabel("ใครเจอบัคก็เจอไงครับงงไร")
